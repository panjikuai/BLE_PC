...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario1_Discovery : Page
    {
        ...
        public const string HID_GUID_STR = "00001812-0000-1000-8000-00805F9B34FB";
        public readonly Guid HID_GUID = new Guid(HID_GUID_STR);
        ...
        private void EnumerateButton_Click()
        {
            if (deviceWatcher == null)
            {
                ...
                EnumerateButton.Content = "Stop Scanning";
                rootPage.NotifyUser($"Device watcher started.", NotifyType.StatusMessage);
            }
            else
            {
                StopBleDeviceWatcher();
                EnumerateButton.Content = "Start Scanning";
        ...
        private void StartBleDeviceWatcher()
        {
            ...
            //string aqsAllBluetoothLEDevices = "(" + GattDeviceService.GetDeviceSelectorFromUuid(HID_GUID) + ")";
        ...
        //
        private bool unpairingIsBusy = false;
        private async void UnpairButton_Click()
        {
            // Do not allow a new Pair operation to start if an existing one is in progress.
            if (unpairingIsBusy)
            {
            ...
            if (bleDeviceDisplay != null)
            {
                ...
                // BT_Code: Pair the currently selected device.
                DeviceUnpairingResult result = await bleDeviceDisplay.DeviceInformation.Pairing.UnpairAsync();
                rootPage.NotifyUser($"Pairing result = {result.Status}",
                    result.Status == DeviceUnpairingResultStatus.Unpaired || result.Status == DeviceUnpairingResultStatus.AlreadyUnpaired
                        ? NotifyType.StatusMessage
                        : NotifyType.ErrorMessage);
            }
            else
            {
                rootPage.NotifyUser("Please select the paired device!", NotifyType.StatusMessage);
            }

            unpairingIsBusy = false;
        }
