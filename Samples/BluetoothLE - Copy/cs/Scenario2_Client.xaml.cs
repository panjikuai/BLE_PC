...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario2_Client : Page
    {
        ...
        private async void ConnectButton_Click()
        {
            ...
            if (bluetoothLeDevice != null)
            {
                ...
                if (result.Status == GattCommunicationStatus.Success)
                {
                    ...
                    {
                        svcGuid = service.Uuid.ToString().ToUpper();
                        ServiceCollection.Add(new BluetoothLEAttributeDisplay(service));
                        if (svcGuid == Scenario1_Discovery.HID_GUID_STR)
                        {
                            rootPage.NotifyUser("Found HID:"+svcGuid, NotifyType.StatusMessage);
                        }
                    }
        ...
        private async void ServiceList_SelectionChanged()
        {
            ...
            try
            {
                ...
                if (accessStatus == DeviceAccessStatus.Allowed)
                {
                    ...
                    {
                        rootPage.NotifyUser("Error accessing service.ERROR:"+ accessStatus.ToString(), NotifyType.ErrorMessage);
            ...
            {
                rootPage.NotifyUser(c.Uuid.ToString().ToUpper(), NotifyType.StatusMessage);
        ...
        private void CharacteristicLatestValue_Copy_KeyDown(object sender, Windows.UI.Xaml.Input.KeyRoutedEventArgs e)
        {
        ...
        {
            string str = null;
            str += "Key:" + e.OriginalKey.ToString();
            str += " value:" + Convert.ToInt32(e.OriginalKey).ToString();
            str += " IsKeyReleased:" + e.KeyStatus.IsKeyReleased.ToString();
